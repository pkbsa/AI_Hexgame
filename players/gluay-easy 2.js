/*
Member : 
  6388093 Nitit Ngamphotchanamongkol
  6388117 Siranut Akarawuthi
  6388206 Tawan Meethong

*This Bot Work Best on 5x5 Board

Dept Limit
  5x5 Board -> Maximum Deptlimit = 4
  7x7 Board -> Maximum Deptlimit = 3
  11x11 Board -> Maximum Deptlimit = 2
*/
...
class Agent {
  ...
  searchMove(initial_state, keepBestMove) {
    ...
    console.log(actions.length, this.depthLimit)
  ...
  maxVal(state, depth, alpha, beta) {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      alpha = Math.min(alpha, minimaxVal)
      if (beta <= alpha) {
        //console.log("prunning")
        break
      }
  ...
  // https://github.com/cmaraziaris/Hex-playing-AI
  // Calculate Score using isBridge Function
  evaluate(state) {
    let i,j;
    let count = 0;
    let br;
    for (j=0; j < state.hex_size; ++j){
      for(i=0; i < state.hex_size; ++i){
        br = isBridge(state.board, state.hex_size, i, j, state.player);
        count += br;
      }
    }
    return count;
...
//https://github.com/cmaraziaris/Hex-playing-AI
function isBridge(table, size, i, j, color){
  let count = 0;
  let signR, signB, nonsignR, nonsignB;

  signR = signB = nonsignR = nonsignB = 0; 
  if(color = RED){
    signR = 3;
    nonsignR = 1.5;
  }
  ...
  if(i+1 < size){
    if(j-2 >= 0)
      if (table[i+1][j-2] == color && table[i+1][j-1] == EMPTY && table[i][j-1] == EMPTY)
        count += signB + nonsignR
    if (j+1 < size)
      if (table[i+1][j+1] == color && table[i][j+1] == EMPTY && table[i+1][j] == EMPTY)
        count += nonsignR + nonsignB;
  }
  if (i + 2 < size && j - 1 >= 0)
    if (table[i+2][j-1] == color && table[i+1][j] == EMPTY && table[i+1][j-1] == EMPTY)
      count += signR + nonsignB; 
  //console.log(count)
  return count;
}